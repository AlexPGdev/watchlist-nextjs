    ...
    useEffect(() => {
        ...
        if(effectiveQuery.startsWith("@")) {
            ...
            const debounceTimeout = setTimeout(async () => {
                try {
                    const response = await fetch(`http://192.168.178.138:8080/api/search?q=${encodeURIComponent(effectiveQuery.replace('@', ''))}`)
        ...
        const debounceTimeout = setTimeout(async () => {
            try {
                const response = await fetch(`http://192.168.178.138:8080/api/content/searchContent?query=${encodeURIComponent(effectiveQuery.trim())}${yearFilter ? `&year=${yearFilter}` : ''}`)
                const data = await response.json()
