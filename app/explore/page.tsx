          ...
          {recommendations.length > 0 && (
            recommendations.map((item, index) => (
              item.cacheKey !== 'trending_movies' && (
                <div key={item.cacheKey} className="flex flex-col select-none overflow-x-scroll no-scrollbar overflow-y-visible">
                  <div>              
                  ...
                  <div className="flex gap-2 [mask-image:linear-gradient(to_right,black_90%,transparent)] py-2">
                    {item.objects.map((object, index) => (
                      <div key={object.movie ? object.movie.id : object.id} className="grid gap-2" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))' }}>
